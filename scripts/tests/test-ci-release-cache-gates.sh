...
require_text "$RELEASE_WORKFLOW" "cargo build --release --target \${{ matrix.target }} --package y-cli --package y-gui --bins --features y-gui/custom-protocol"
require_text "$RELEASE_WORKFLOW" "npx @tauri-apps/cli bundle --ci --target \${{ matrix.target }}"
require_text "$RELEASE_WORKFLOW" "npx @tauri-apps/cli bundle --ci --bundles app --target \${{ matrix.target }}"
reject_text "$RELEASE_WORKFLOW" "npx @tauri-apps/cli build --ci --target \${{ matrix.target }}"
require_text "$RELEASE_WORKFLOW" "scripts/package-macos-pkg.sh"
require_text "$RELEASE_WORKFLOW" "dist/*.pkg"
