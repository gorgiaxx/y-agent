...
export const COMMAND_MAP: Record<string, EndpointDef> = {
  ...
  session_get_prompt_config: { method: 'GET', path: pathWith('/api/v1/sessions/{sessionId}/prompt-config', 'sessionId') },
  session_set_prompt_config: { method: 'PUT', path: pathWith('/api/v1/sessions/{sessionId}/prompt-config', 'sessionId'),
                         body: (a) => ({ config: a.config }) },
  ...
  chat_request_plan_revision: { method: 'POST', path: '/api/v1/chat/request-plan-revision',
                         body: (a) => ({ plan_run_id: arg(a, 'planRunId', 'plan_run_id'), feedback: arg(a, 'feedback') }) },
  ...
  session_list_plan_runs: { method: 'GET', path: pathWith('/api/v1/chat/plan-runs/{sessionId}', 'sessionId') },
  chat_add_follow_up:  { method: 'POST', path: '/api/v1/chat/follow-up',
                         body: (a) => ({ session_id: arg(a, 'sessionId', 'session_id'), text: arg(a, 'text') }) },
  chat_steer_follow_up: { method: 'POST', path: '/api/v1/chat/follow-up/steer',
                          body: (a) => ({ session_id: arg(a, 'sessionId', 'session_id'), follow_up_id: arg(a, 'followUpId', 'follow_up_id') }) },
  chat_unsteer_follow_up: { method: 'DELETE', path: '/api/v1/chat/follow-up/steer',
                          body: (a) => ({ session_id: arg(a, 'sessionId', 'session_id'), follow_up_id: arg(a, 'followUpId', 'follow_up_id') }) },
  ...
  prompt_template_list: { method: 'GET', path: '/api/v1/config/prompt-templates' },
  prompt_template_save: { method: 'PUT', path: pathWith('/api/v1/config/prompt-templates/{id}', 'id'),
                         body: (a) => ({ template: a.template }) },
  prompt_template_delete: { method: 'DELETE', path: pathWith('/api/v1/config/prompt-templates/{id}', 'id'), response: successResponse },
  // -- Knowledge --
