//! First-run initialization: seed config, prompts, skills, agents, and persona.
//!
...
// Persona template files (embedded at compile time).
const PERSONA_TOML: &str = include_str!("../../../config/persona/persona.toml");
const PERSONA_SOUL: &str = include_str!("../../../config/persona/SOUL.md");
const PERSONA_IDENTITY: &str = include_str!("../../../config/persona/IDENTITY.md");
const PERSONA_USER: &str = include_str!("../../../config/persona/USER.md");
const PERSONA_MEMORY: &str = include_str!("../../../config/persona/MEMORY.md");
const PERSONA_BOOTSTRAP: &str = include_str!("../../../config/persona/BOOTSTRAP.md");
...
/// Persona template files to seed into `<config_dir>/persona/`.
const PERSONA_TEMPLATES: &[(&str, &str)] = &[
    ("persona.toml", PERSONA_TOML),
    ("SOUL.md", PERSONA_SOUL),
    ("IDENTITY.md", PERSONA_IDENTITY),
    ("USER.md", PERSONA_USER),
    ("MEMORY.md", PERSONA_MEMORY),
    ("BOOTSTRAP.md", PERSONA_BOOTSTRAP),
];
...
pub struct InitReport {
    ...
    /// Persona template files that were seeded.
    pub persona_seeded: Vec<String>,
...
pub fn ensure_initialized(
    ...
    report.persona_seeded = seed_persona_templates(config_dir)?;
    ...
    report.was_first_run = !report.configs_created.is_empty()
        ...
        || !report.persona_seeded.is_empty();
    ...
    if report.was_first_run {
        info!(
            ...
            persona = report.persona_seeded.len(),
...
/// Seed persona template files into `<config_dir>/persona/`.
///
/// Creates the `persona/` directory and writes all template files.
/// Existing files are never overwritten (the bot may have modified them).
fn seed_persona_templates(config_dir: &Path) -> Result<Vec<String>> {
    let persona_dir = config_dir.join("persona");
    std::fs::create_dir_all(&persona_dir)
        .with_context(|| format!("creating persona directory: {}", persona_dir.display()))?;

    let mut seeded = Vec::new();

    for &(filename, content) in PERSONA_TEMPLATES {
...
mod tests {
    ...
    fn ensure_initialized_creates_all_files() {
        ...
        assert!(!report.persona_seeded.is_empty());
        ...
        assert!(config_dir.join("persona").is_dir());
        assert!(config_dir.join("persona/persona.toml").exists());
        assert!(config_dir.join("persona/SOUL.md").exists());
        assert!(config_dir.join("persona/IDENTITY.md").exists());
        assert!(config_dir.join("persona/USER.md").exists());
        assert!(config_dir.join("persona/MEMORY.md").exists());
        assert!(config_dir.join("persona/BOOTSTRAP.md").exists());
    }
    ...
    fn ensure_initialized_is_idempotent() {
        ...
        assert!(report2.persona_seeded.is_empty());
    }
    ...
    #[test]
    fn seed_persona_templates_creates_all_files() {
        let tmp = tempfile::TempDir::new().unwrap();
        let config_dir = tmp.path().join("config");
        std::fs::create_dir_all(&config_dir).unwrap();

        let seeded = seed_persona_templates(&config_dir).unwrap();
        assert_eq!(seeded.len(), PERSONA_TEMPLATES.len());

        // All files should exist.
        for &(filename, _) in PERSONA_TEMPLATES {
            assert!(
                config_dir.join("persona").join(filename).exists(),
                "expected persona/{filename} to exist"
            );
        }

        // Second run should be idempotent.
        let seeded2 = seed_persona_templates(&config_dir).unwrap();
        assert!(seeded2.is_empty());
    }

    #[test]
    fn seed_persona_templates_does_not_overwrite_existing() {
        let tmp = tempfile::TempDir::new().unwrap();
        let config_dir = tmp.path().join("config");
        let persona_dir = config_dir.join("persona");
        std::fs::create_dir_all(&persona_dir).unwrap();

        // Pre-create a modified SOUL.md.
        std::fs::write(persona_dir.join("SOUL.md"), "Custom soul").unwrap();

        let seeded = seed_persona_templates(&config_dir).unwrap();

        // SOUL.md should NOT be in seeded (was not overwritten).
        assert!(!seeded.contains(&"SOUL.md".to_string()));
        // But other files should be seeded.
        assert!(seeded.contains(&"persona.toml".to_string()));
