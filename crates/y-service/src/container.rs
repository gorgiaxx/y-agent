...
use y_diagnostics::{DiagnosticsSubscriber, InMemoryTraceStore, TraceStore};
...
pub struct ServiceContainer {
    ...
    pub provider_pool: Arc<ProviderPoolImpl>,
...
"#).expect("fallback taxonomy")
        ...
        // 12. Workflow store.
        let workflow_store = SqliteWorkflowStore::new(pool);

        // 13. Diagnostics.
        let trace_store: Arc<dyn TraceStore> =
            match wire_trace_store(&config.storage).await {
                Ok(store) => store,
                Err(e) => {
                    warn!(error = %e, "Failed to initialize PG diagnostics; using in-memory store");
                    Arc::new(InMemoryTraceStore::new())
                }
            };
        let diagnostics = Arc::new(DiagnosticsSubscriber::new(trace_store));
        ...
        match config.provider_type.as_str() {
            ...
            other => {
                warn!(
                    provider_id = %config.id,
                    provider_type = %other,
                    "Skipping provider: unsupported type (supported: openai, anthropic, gemini, ollama, azure)"
                );
            }
        }
    }
...
/// Attempt to create a PG-backed trace store from config.
async fn wire_trace_store(
    storage_config: &y_storage::StorageConfig,
) -> Result<Arc<dyn TraceStore>> {
    if storage_config.postgres_url.is_none() {
        info!("No postgres_url configured; diagnostics will use in-memory store");
        return Ok(Arc::new(InMemoryTraceStore::new()));
    }

    #[cfg(feature = "diagnostics_pg")]
    {
        let pg_pool = y_storage::create_pg_pool(storage_config)
            .await
            .context("failed to create PG pool for diagnostics")?;

        let store = y_diagnostics::PgTraceStore::new(pg_pool);
        store
            .run_migrations()
            .await
            .context("failed to run diagnostics PG migrations")?;

        info!("Diagnostics: PostgreSQL trace store initialized");
        Ok(Arc::new(store) as Arc<dyn TraceStore>)
    }

    #[cfg(not(feature = "diagnostics_pg"))]
    {
        warn!("postgres_url set but diagnostics_pg feature not enabled; using in-memory store");
        Ok(Arc::new(InMemoryTraceStore::new()))
    }
}
