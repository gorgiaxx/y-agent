...
///
/// Passed through to the provider's image generation API. Fields map to
/// the Doubao/Seedream `extra_body` parameters.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ImageGenerationOptions {
    ...
    /// When > 1, sequential image generation is enabled automatically.
